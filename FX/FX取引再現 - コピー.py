...
def ask_settlement(money,reward):  # 売りポジションを保有している時の行動　決済(買い)するか、しないか)
    ...
    if bid_val:  # 売り注文リストに要素が入っているとき
        ...
        if random_act == ask_act:  # 保有している中で1番目の要素を決済する
...
for all_iteration_index in range(max_epi): # max_epi回繰り返す (max_epi回のイテレーション)
    ...
    all_final_ask_rewards = discount_and_normalize_rewards(ask_reward, discount_factor)

    all_mean_ask_grads = []
    for var_index in range(len(model.trainable_variables)):
        mean_ask_grads = tf.reduce_mean(
            ask_reward * ask_grads[episode_index][var_index]
